...
v0.7.0부터 ASI판을 기본 게시합니다. 기존 v0.6.2 이하 릴리즈의 DLL ZIP은 변경하지 않습니다.
...
v0.7.0 ASI판에서는 **Ctrl+Alt+D → 전장 비율 → 4:3 / 16:9**를 선택할 수 있습니다.
저장 후 게임을 재실행하면 적용됩니다. 기본값은 4:3이며, 16:9는 전장을 1068×600으로
확장하고 800픽셀 HUD를 가운데 배치합니다. 원본 EXE 파일은 수정하지 않습니다.
v0.7.0은 검증한 ESL 2606 원본만 지원합니다. v0.7.1부터는 실행 코드·데이터·주소 배치가 같고
아이콘·버전 등의 리소스만 변경된 EXE도 지원합니다. 게임 코드·데이터 변경과 주소 이동은 지원하지 않습니다.
SYW2X의 전장 확장은 꺼 두어야 합니다. 16:9 요청에 실패하면 오버레이에 이유를 안내합니다.
...
창 크기는 테두리·모서리를 드래그해 바꿀 수 있으며, 제목 표시줄을 제외한 화면 영역의 비율을 유지합니다.
800×600 모드는 4:3, 1068×600 와이드 모드는 16:9로 조절합니다. SYW2X 확장 모드는 실행 해상도를 따릅니다.
비율 설정을 저장해도 재실행 전에는 현재 실행 중인 비율을 유지합니다. 최대화는 모니터 작업 영역을 사용하고 게임 영상의 비율을 유지합니다.
