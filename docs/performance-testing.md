...
### 전장 스크롤 확인 (2026-09-11)

`output/perf-battle-scroll-20260911/run-00`에서 정확한 가장자리 좌표로 카메라 스크롤을
재현했다. 이 실행의 창 크기는 1224×959, 클라이언트는 창 내부 (12,47)부터 1200×900이었다.
창 내부 x=1209는 게임 x=798에 대응하여 스크롤이 시작되지 않았지만, x=1211은 x=799에
대응하여 오른쪽 녹색 화살표와 기지·지형의 이동이 확인됐다(`exact-right-edge.png`).
왼쪽 가장자리는 x=12였다. 이 좌표를 다른 창 크기나 DPI에 그대로 사용하면 안 된다.

게임 좌표의 끝 열에서 스크롤이 시작되는 것이 관찰된 사실이며, 이것만으로 래퍼의
좌표 변환 결함이라고 판단하지 않는다. 기존 키/미니맵 시도와 달리 실제 카메라 이동을
눈으로 확인했지만, 오래 유지하면 미탐색 지역과 지도 끝에 도달하므로 지속 지형 이동과 구분한다.

이후 GPU + Sharp Bilinear + VSync OFF 창 모드에서 120초 수집했다. 2414회 출력 호출을
수집했으며 손실 없음·포커스 유지·GPU 유지·정상 종료·측정 종료 포함 검사를 통과했다.

| 120초 가장자리 유지/정지 혼합 표본 | 결과 |
| --- | ---: |
| 출력 호출 간격 p95 / p99 / 최대 | 51.6498 / 52.3849 / 53.5698 ms |
| Present 호출 CPU p95 / 최대 | 0.1987 / 0.4096 ms |
| 전체 출력 처리 CPU p95 / 최대 | 0.4043 / 0.7658 ms |
| mouse_lock_wait 최대 | 0.0006 ms |
| 프로세스 CPU (논리 코어 하나 = 100%) | 99.46% |

`segments.json`에 좌측 유지·정지·우측 유지 구간을 분리했다. 각 구간의 출력 간격 최댓값은
52.9664 / 52.8072 / 53.5698ms였다. 미탐색 지역과 지도 경계가 포함되므로 이 구간 전체를
지속적으로 지형을 그리는 스크롤 부하라고 해석하지 않는다. 본 수집 시작 전, 화면 증거가
있는 최초 우측 스크롤 조작 구간도 보조 분석했다. 해당 짧은 구간의 29개 호출 간격 중
최대는 53.9662ms, Present 최대는 0.3674ms였다. 이는 본 120초 표본과 합산하지 않는다.
조작/화면 캡처 비용이 포함되어 인과관계와 실제 커서 표시 지연은 여전히 확정할 수 없다.
이번 재현에서 큰 순간 멈춤은 관찰되지 않았으며 대규모 교전 검증은 남아 있다.
