...
ESL의 기존 ASI 로더와 SYW2X를 보존하는 구성이다. 현재 개발 소스에 포함되어 있으며
...
새 ASI 빌드는 첫 DirectDraw 생성 시 원본 ESL 2606 EXE의 SHA-256과 167개 패치 위치를
검사한다. `[Display] BattleAspect=16:9`일 때만 전장/HUD/지형 캐시 코드를 메모리에
적용하며, 기본값 `4:3`에서는 이 패치를 쓰지 않는다. 디스크의 EXE는 그대로 유지한다.
메뉴와 로비는 기존 모드를 사용한다. 비율 변경은 저장 후 재실행해야 적용된다.

캐시·중계 코드는 Windows가 선택한 빈 주소에 할당한다. 코드 페이지는 RX, 캐시는 RW로
분리하고 원본 코드의 보호 속성을 복원한다. 주소가 바뀌면 78개 패치 참조와 11개 중계
코드 참조를 재배치한다. 모든 원본 바이트와 쓰기 권한을 확보한 뒤 적용하므로 불일치나
권한 실패 시 일부만 적용하지 않는다. 현재 SYW2X `ViewPortPlusOn=1`과의 병용은 거부한다.

167개 패치와 중계 코드는 `src/widescreen_recipe.h`에 들어 있다. CI는 원본 게임 없이
빌드한다. 개발용 `tools/export_widescreen_patch.py`는 해시 검증된 원본과 실험 파일을
대조하여 이 명세를 생성한다. 실험 EXE와 게임 자산은 배포물에 포함하지 않는다.

### DirectDraw 연결

`DllMain`은 모듈 핸들만 기록한다. ASI 로더가 `InitializeASI`를 호출하면 최초 한 번만
게임 EXE의 명명된 `DDRAW.dll!DirectDrawCreateEx` IAT 슬롯을 연결한다.
실행 파일의 디스크 내용, 다른 모듈의 IAT, 전역 API 진입점, 게임 타이머는 변경하지 않는다.
위의 16:9 실험 옵션을 켜면 확인된 게임 코드 일부를 프로세스 메모리에서만 변경한다.
