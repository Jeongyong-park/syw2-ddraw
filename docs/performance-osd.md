...
패널은 마우스 클릭과 키보드 포커스를 받지 않습니다. 디스플레이 설정을 열거나 게임이 비활성화·최소화되면 숨겨지고 진행 중인 벤치마크가 종료됩니다. 다시 게임으로 돌아오면 패널과 마지막 결과가 표시됩니다. 설정을 적용하거나 Alt+Enter로 화면 모드를 전환하면 측정 조건이 바뀌므로 이전 결과를 지웁니다.
