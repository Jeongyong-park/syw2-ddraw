...
LRESULT CALLBACK window_proc(HWND h, UINT msg, WPARAM w, LPARAM l, UINT_PTR, DWORD_PTR data) {
    ...
    if (msg==WM_SIZING && d->windowed && !d->layout_busy && l) {
        ...
        if(GetWindowRect(h,&outer) && GetClientRect(h,&client) &&
           hq::constrain_window_aspect(*reinterpret_cast<RECT*>(l),w,
...
HRESULT Draw::SetCooperativeLevel(HWND h, DWORD flags) {
    ...
    window=h;
    hq::overlay_input::install([] { return settings_window!=nullptr; });
    shortcut_draw=this;
