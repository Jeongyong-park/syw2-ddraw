...
struct Viewport {
    ...
    static Viewport fit(int client_w, int client_h, int game_w, int game_h, bool integer=false) {
        ...
        if(integer) {
            const int factor=std::min(client_w/game_w,client_h/game_h);
            // Below 1x, aspect-fit downscaling avoids clipping the game.
            if(factor>=1) { w=game_w*factor; h=game_h*factor; }
        }
