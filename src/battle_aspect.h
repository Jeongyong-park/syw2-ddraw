...
namespace hq {
// Experimental EXEs carry reversible, preimage-checked patches. Apply once,
// before the first battle; never change live surfaces or HUD coordinates.
struct BattleAspect {
    bool available=false, wide=false;
    const char* reason="legacy prototype or unsupported executable";
    static bool requested(const wchar_t* value) { return std::wcscmp(value,L"16:9")==0; }
    ...
    // Separate image argument permits native tests on a private mapped fixture.
